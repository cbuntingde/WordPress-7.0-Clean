	...
	self.SidebarPartial = api.selectiveRefresh.Partial.extend(
		/** @lends wp.customize.widgetsPreview.SidebarPartial.prototype */        {
			...
			ensureWidgetPlacementContainers: function ( widgetId ) {
				...
				if ( ! widgetPartial ) {
					widgetPartial = new self.WidgetPartial(
						partialId,
						{
							params: {}
						}
	...
	self.addPartials = function () {
		_.each(
			...
			function ( registeredSidebar ) {
				...
				if ( ! partial ) {
					partial = new self.SidebarPartial(
						partialId,
						{
							params: {
